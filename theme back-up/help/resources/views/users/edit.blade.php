						...
						<div class="row">
								...
								<div class="form-group @if($errors->has('company_id')) has-error @endif">
									...
									<span class="block input-icon input-icon-right">
										<select class="width-100" name="company_id">
											...
											<option value="{{$r->id}}" @if($r->id==$user->company_id) selected @endif>{{$r->company_name}}</option>
												@endforeach
											@endif
										</select>
							...
							<div class="col-xs-12 col-sm-6">
								<div class="form-group @if($errors->has('branch_id')) has-error @endif">
									<label>Branch</label>
									<span class="block input-icon input-icon-right">
										<select class="width-100" name="branch_id">
											<option value="">Select Branch</option>
											@if($branch)
												@foreach($branch as $r)
											<option value="{{$r->id}}" @if($r->id==$user->branch_id) selected @endif>{{$r->branch_name}}</option>
												@endforeach
											@endif
										</select>
